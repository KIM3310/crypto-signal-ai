# Conversion UX Model - Crypto Signal AI
...
| Primary buyer or user | Quant learners, research communities, analytics teams, and automation builders. |
| Value wedge | Auditable signal research with indicators, sentiment hooks, and backtesting, without return promises. |
...
| First action | Open the portfolio-fit note, confirm the successor project, then inspect Tests pass only if this domain is relevant. |
...
- Optional entry: Paid research dashboard ($9-$29/month research access) when this archived proof exactly matches the buyer problem.
- Revival step: Private strategy-lab template ($1k-$5k setup) only after the active successor and support boundary are clear.
- Longer-term path: Automation/report workflow setup ($500-$3k/month private reporting) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Report retention only as a discussion metric until fresh validation exists.
